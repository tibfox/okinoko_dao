...
func TestBreak_NoOptionWinningStillPaysOut(t *testing.T) {
	...
	assertAborts(t, exec, "proposal is failed", "a down-voted (NO) proposal must not be executable")
...
func TestBreak_QuorumInflationViaMultiSelect(t *testing.T) {
	...
	assertAborts(t, exec, "proposal is failed", "quorum (2 of 3) met by a SINGLE multi-selecting voter")
...
func TestBreak_NegativePayoutRejected(t *testing.T) {
	...
	assertAborts(t, res, "payout amount must be positive", "proposal with a NEGATIVE payout amount was accepted")
}
...
func TestBreak_ZeroVotesFailsAndCannotExecute(t *testing.T) {
	...
	rawCallAt(ct, "proposal_tally", PayloadUint64(propID), nil, "hive:someone", lateTS, "t")
	exec := rawCallAt(ct, "proposal_execute", PayloadString(fmt.Sprintf("%d", propID)), nil, "hive:someone", lateTS, "e")
	assertAborts(t, exec, "proposal is failed", "proposal with zero votes must not be executable")
}
...
func TestBreak_ReVoteSameOptionDoesNotDoubleWeight(t *testing.T) {
	...
	exec := rawCallAt(ct, "proposal_execute", PayloadString(fmt.Sprintf("%d", propID)), nil, "hive:someone", lateTS, "e")
	after := hiveBal(ct, "hive:someoneelse")
	assertAborts(t, exec, "proposal is failed", "re-voting doubled a sub-threshold voter over the line")
...
func TestBreak_EmptyChoiceVoteDoesNotCountQuorum(t *testing.T) {
	...
	exec := rawCallAt(ct, "proposal_execute", PayloadString(fmt.Sprintf("%d", propID)), nil, "hive:someone", lateTS, "e")
	after := hiveBal(ct, "hive:someoneelse")
	assertAborts(t, exec, "proposal is failed", "empty ballot counted toward quorum")
...
func TestBreak_VoteInvalidOptionIndex(t *testing.T) {
	...
	assertAborts(t, res, "invalid option index", "vote for nonexistent option index was accepted")
	...
	assertAborts(t, inRange, "invalid option index",
...
func TestBreak_VoteAfterLeaveRejected(t *testing.T) {
	...
	assertAborts(t, res, "hive:someoneelse is not a member", "ex-member was allowed to vote")
}
...
func TestBreak_ProposalDurationOverflow(t *testing.T) {
	...
	res := rawCallAt(ct, "proposal_create", PayloadString(joinPipe(fields)), transferIntent("1.000"), "hive:someone", defaultTimestamp, "c")
	if !res.Success {
		// Rejecting the absurd duration outright is the correct outcome, and is what
		// happens today. Assert WHY, so this cannot silently start failing for an
		// unrelated reason — the tally branch below then becomes unreachable, which
		// is why it must not be the only thing this test checks.
		assertAborts(t, res, "proposal duration must not exceed",
			"a max-uint64 proposal duration")
		return
	}
	...
	assertAbortsAny(t, tally, "duration overflow let a proposal tally immediately")
}
...
func TestBreak_AddFundsWhilePausedBlocked(t *testing.T) {
	...
	assertAborts(t, res, "cannot add stake while project is paused", "AddFunds succeeded while project was paused (Join/Leave are blocked)")
}
...
func TestBreak_MultiAssetPayoutAtomicRollback(t *testing.T) {
	...
	exec := rawCallAt(ct, "proposal_execute", PayloadString(fmt.Sprintf("%d", propID)), nil, "hive:someone", lateTS, "e")
	after := hiveBal(ct, "hive:someoneelse")
	assertAborts(t, exec, "insufficient hbd funds in treasury", "payout with an unfunded 2nd asset should abort")
...
func TestGuard_WhitelistOverlongAddressRejected(t *testing.T) {
	...
	assertAborts(t, res, "address exceeds maximum length", "whitelist accepted an over-length address")
}
...
func TestGuard_PayoutOverlongAddressRejected(t *testing.T) {
	...
	res := rawCallAt(ct, "proposal_create", PayloadString(joinPipe(fields)), transferIntent("1.000"), "hive:someone", defaultTimestamp, "c")
	assertAborts(t, res, "address exceeds maximum length", "payout accepted an over-length address")
}
...
func TestGuard_WhitelistSpaceAddressRejected(t *testing.T) {
	...
	assertAborts(t, res, "invalid character in address", "whitelist accepted an address with an internal space")
}
...
func TestGuard_TransferOwnershipBadAddressRejected(t *testing.T) {
	...
	assertAborts(t, res, "invalid character in address", "ownership transfer accepted an address containing ';'")
}
...
func TestGuard_OverflowDepositRejected(t *testing.T) {
	...
	assertAborts(t, res, "amount out of range", "an overflowing deposit was accepted")
}
...
func TestBound_DoubleTallyRejected(t *testing.T) {
	...
	assertAborts(t, second, "proposal not active", "a proposal was tallied twice")
}
...
func TestBound_TallyBeforeDeadlineRejected(t *testing.T) {
	...
	assertAborts(t, res, "proposal still running until 2025-09-03T01:00:00Z", "proposal tallied before its deadline")
}
...
func TestBound_ExecuteWithoutTallyRejected(t *testing.T) {
	...
	assertAborts(t, res, "proposal is active", "an un-tallied proposal was executed")
}
...
func TestBound_NonOwnerCannotPause(t *testing.T) {
	...
	assertAborts(t, res, "only owner can pause/unpause", "a non-owner paused the project")
}
...
func TestBound_DoubleJoinRejected(t *testing.T) {
	...
	assertAborts(t, res, "already a member", "a member joined the same project twice")
}
...
func TestBound_CancelAfterTallyRejected(t *testing.T) {
	...
	assertAborts(t, res, "proposal not active", "a tallied proposal was cancelled")
}
...
func TestBound_NonMemberCannotCreateProposal(t *testing.T) {
	...
	assertAborts(t, res, "only members can create proposals", "a non-member created a proposal in a members-only DAO")
}
...
func TestBound_PayoutTargetLockedOnlyAfterApproval(t *testing.T) {
	...
	assertAborts(t, blocked, "active proposal requesting funds", "beneficiary left while an APPROVED payout was pending")
...
func TestBound_OwnerCannotLeave(t *testing.T) {
	...
	assertAborts(t, res, "owner must transfer ownership before leaving", "owner left without transferring ownership")
}
...
func TestBound_ProposalCostEnforced(t *testing.T) {
	...
	assertAborts(t, res, "proposal cost requires at least 1.000000 hive", "proposal created with an intent below the required cost")
}
...
func TestBound_DemocraticJoinRejectsOverpay(t *testing.T) {
	...
	assertAborts(t, res, "democratic projects require exactly 1.000000 hive", "democratic DAO accepted a deposit != stakeMin")
}
...
func TestBound_WhitelistStillRequiresStake(t *testing.T) {
	...
	assertAborts(t, res, "no valid transfer intent provided", "whitelisted member bypassed the stake deposit")
}
